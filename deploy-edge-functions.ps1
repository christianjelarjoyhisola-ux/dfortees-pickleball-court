...
$deploymentBrand = Resolve-ConfigValue "DEPLOYMENT_BRAND" $envMap -Required
if ($deploymentBrand -ne "dfortees") {
  throw "DEPLOYMENT_BRAND must be exactly 'dfortees'. Deployment stopped before contacting Supabase."
}
...
try {
  ...
  if ($ApplyMigrations) {
    # Use only after the fresh-database bundle has been applied and reviewed.
    Write-Host "Checking pending database migrations..."
    Invoke-Supabase db push --dry-run
    Invoke-Supabase db push
  } else {
    Write-Host "Database migration push skipped. Pass -ApplyMigrations only after reviewing the target project."
  }
  ...
  Write-Host "D'fortees Edge Functions deployed successfully."
} finally {
  Pop-Location
}
