...
const _sb = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  global: {
    fetch: PB_BACKEND_CONFIGURED
      ? (input, init) => _pbFetchWithTimeout(input, init)
      : async () => { throw new Error("D'fortees backend is not configured. Local demo mode is active."); },
  },
});
...
window.PB_USE_LOCAL_DATA = !PB_BACKEND_CONFIGURED
...
async function _pbVerifyReceiptBase64Fallback(fnUrl, payload, imageFile) {
  ...
  const fallbackPayload = {
    ...
    imageBase64,
    ...(payload?.guestAccessToken ? { guestAccessToken: payload.guestAccessToken } : {}),
...
function _bookingEmailPayload(b) {
  const items = Array.isArray(b.items) && b.items.length
    ...
    : Array.isArray(b.groupItems) && b.groupItems.length
      ...
      : [];
  return {
    // Use a concrete row reference for guest-token authorization. The Edge
    // Function derives the public group reference from the database.
    bookingRef: b.ref,
    guestAccessToken: b.guestAccessToken || null,
    email: b.email,
    fullName: b.fullName,
...
// =============================================
// DB â€” Async Data Layer (replaces localStorage)
// =============================================
window.DB = {
  ...
  async verifyGcashReceipt(payload) {
    ...
    if (payload?.imageFile) {
      ...
      if (payload.guestAccessToken) {
        form.append('guestAccessToken', String(payload.guestAccessToken));
      }
  ...
  // Production seed data is owned by versioned SQL migrations. Never create
  // courts from browser code because that would bypass deployment review.
...
// Admin accounts are managed in Supabase Dashboard â†’ Authentication â†’ Users
// The accounts table stores role/display info linked by email.
// =============================================
// =============================================
// LOCAL DATA MODE
// Enable only on localhost with localStorage.setItem('df_data_mode', 'local')
...
(function installLocalDataMode() {
  ...
  console.info('[D’fortees Pickleball Court] Local data mode enabled. Supabase writes are bypassed in this browser.');
})();

window.Auth = {

  // â”€â”€ Role model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // owner       â†’ System Owner   (full access: everything + accounts)
  // court_owner â†’ Court Owner    (operations + payment settings, no account mgmt)
  // staff       â†’ Court Staff    (front-desk: bookings, payment review, open play)
  ...
  async refreshSessionFromAuth({ remember = null } = {}) {
    ...
    const shouldRemember = remember === null ? localStorage.getItem('df_remember') === '1' : !!remember;
    sessionStorage.removeItem('df_session');
    localStorage.removeItem('df_session');
    const store = shouldRemember ? localStorage : sessionStorage;
    store.setItem('df_session', JSON.stringify(session));
    if (shouldRemember) localStorage.setItem('df_remember', '1');
    else localStorage.removeItem('df_remember');
    return session;
  },

  async login(email, password, remember = false) {
    // Sign in via Supabase Auth â€” establishes a verified JWT session.
  ...
  getSession() {
    ...
    const s = localStorage.getItem('df_session') || sessionStorage.getItem('df_session');
    if (!s) return null;
  ...
  async logout() {
    ...
    sessionStorage.removeItem('df_session');
    localStorage.removeItem('df_session');
    localStorage.removeItem('df_remember');
...
if (window.PB_USE_LOCAL_DATA) {
  Object.assign(window.Auth, {
    async login(usernameOrEmail, password, remember = false) {
      ...
      store.setItem('df_session', JSON.stringify(session));
      if (remember) localStorage.setItem('df_remember', '1');
      return { ok: true };
    },
