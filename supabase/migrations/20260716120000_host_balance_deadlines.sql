...
begin
  ...
  if request_role in ('anon', 'authenticated')
    ...
    if new.balance_due_at is distinct from old.balance_due_at
      ...
      raise exception 'Balance deadlines and forfeiture records are managed by D’fortees Pickleball Court.'
        using errcode = '42501';
    end if;
  end if;
  return new;
...
begin
  ...
  perform cron.schedule(
    ...
    $job$select net.http_post(
      url := 'https://dfortees-backend.invalid/functions/v1/process-host-balance-deadlines',
