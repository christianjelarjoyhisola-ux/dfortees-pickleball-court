...
async function requireOwner(req: Request, db: ServiceClient): Promise<OwnerAuthorization> {
  ...
  const ownerAccount = account as { id: string; role: Role } | null;
  if (accountErr || ownerAccount?.role !== "owner") {
    return { ok: false, response: json({ error: "Only system owner can manage accounts" }, 403) };
  }
