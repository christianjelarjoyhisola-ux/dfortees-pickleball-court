...
function emailHtml(info: ReturnType<typeof summary>, eventType: string): string {
  ...
  <table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;background:#fff;border:1px solid #d7e0e6;border-radius:18px;overflow:hidden;box-shadow:0 14px 40px rgba(20,61,99,.12)">
    <tr><td style="background:#0b2744;padding:24px 30px;text-align:center">${LOGO_URL ? `<img src="${LOGO_URL}" width="72" height="72" alt="D’fortees Pickleball Court" style="border-radius:20px;background:#fff;padding:4px">` : ""}<div style="color:#fff;font-weight:900;letter-spacing:2px;margin-top:10px">D’FORTEES</div></td></tr>
    ...
    <tr><td style="padding:30px">
      ...
      ${eventType === "forfeited" ? "" : `<p style="font-size:13px;line-height:1.6;color:#657486;margin:20px 0 0">All payments are final and non-refundable. Contact D’fortees Pickleball Court or follow your original payment instructions to settle the balance.</p>`}
    </td></tr>
    <tr><td style="background:#eaf0f3;padding:16px 30px;text-align:center;color:#657486;font-size:12px">Automated account notice from D’fortees Pickleball Court</td></tr>
