...
const ACCOUNTS = [
  { email: env.OWNER_EMAIL || 'owner@dfortees.local', password: env.OWNER_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'sysowner', full_name: 'System Owner', role: 'owner' },
  { email: env.COURT_OWNER_EMAIL || 'courtowner@dfortees.local', password: env.COURT_OWNER_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'courtowner', full_name: 'Court Owner', role: 'court_owner' },
  { email: env.STAFF_EMAIL || 'staff@dfortees.local', password: env.STAFF_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'courtstaff', full_name: 'Court Staff', role: 'staff' },
];
...
async function run() {
  ...
  for (const acc of ACCOUNTS) {
    ...
    if (authErr) {
      console.log(`  âœ— Auth create [${acc.email}]: ${authErr.message}`);
    ...
    console.log(`  âœ“ Auth user created: ${acc.email} (id: ${uid})`);
...
async function upsertAccountRow(uid, acc) {
  ...
  if (error) console.log(`  âœ— accounts table insert [${acc.email}]: ${error.message}`);
  else console.log(`  âœ“ accounts row inserted: ${acc.username} (${acc.role})`);
}
